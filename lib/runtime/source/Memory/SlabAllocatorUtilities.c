    ...
    )
{
    ...
    // slab for the free map, since the slab itself is tiny.
    //
    ...
    Status = PA_ALLOCATE (Cache->BaseAllocator, (VOID**)&Slab, SlabPages + SlabDataPages, PAGE_SIZE_4K);
    if (FAILED (Status)) {
        goto Exit;
    }
    ...
    RtlFillMemory (Slab->FreeMap, Slab->FreeMapEnd - Slab->FreeMap, 0xFF);
 ...
 * @param[out] Bit    The bit of the object within the free map
 *
    ...
    )
{
    ...
    //
    // Search the free map for an empty object
    //
